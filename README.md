...
브리핑이 매일 아침 혼자 돌려면 권한을 미리 열어둬야 합니다. 매일 눌러줄 필요는 없고, 만든
직후에 두 가지만 확인하면 됩니다. 사이드바 **루틴**에서 브리핑을 눌러 상세 페이지를 열어보세요.

<img width="420" alt="루틴 상세 페이지 — 지금 실행 버튼, 폴더, 항상 허용됨 목록" src="images/routine-detail-page.png" />

1. **폴더가 위키 폴더인지.** 상위 폴더로 잡혀 있으면 위키의 `.claude/settings.json`이 읽히지
   않고, 명령마다 `cd`가 붙어 복합 명령이 되는 바람에 "항상 허용" 선택지가 아예 안 뜹니다.
   그러면 실행할 때마다 계속 물어봅니다.
2. **"지금 실행"을 한 번 눌러 보기.** 권한을 묻는 창이 뜨면 **항상 허용**을 고르세요. 다음
   실행부터 같은 도구는 묻지 않습니다. 저장된 승인은 상세 페이지 **항상 허용됨**에서 확인하고
   지울 수 있습니다.

입력창 옆의 **권한 무시**는 안전장치를 통째로 끄는 모드입니다. 공식 문서도 컨테이너나 VM에서만
쓰라고 하니, 매일 켜는 습관은 들이지 마세요.
