...
2. **데일리 논문 브리핑** — 관심사를 바탕으로 매일 정해진 시간에 PubMed 최신 논문을 훑어
   브리핑합니다. PubMed는 생명과학·의학 문헌만 다루므로, 재료과학이나 ML처럼 바깥에 있는
   분야는 세팅 중에 검색 소스를 arXiv로 바꿔 잡아줍니다. Claude의 **루틴** 기능으로 돌아갑니다 — 세팅할 때 Claude가 루틴을 대신
   등록해 주고, 그 뒤로는 사이드바의 *루틴*에서 시간을 바꾸거나 잠시 꺼둘 수 있습니다. 수신
   방법은 데스크탑 알림(기본, 설정 불필요), 이메일, Slack, Notion 중 선택. 루틴은 **앱이 켜져
   있고 컴퓨터가 깨어 있을 때** 돕니다 — 노트북이 밤새 잠들어 있었다면 아침 브리핑은 그냥
   건너뜁니다. 설정 → 데스크탑 앱 → 일반에서 *컴퓨터 깨어 있게 유지*를 켜두면 됩니다(덮개를
   닫으면 그래도 잠듭니다). 놓친 회차는 컴퓨터가 깨어날 때 한 번 따라잡고, 그래도 빠진 날은
   위키 폴더에서 Claude를 열면 밀린 브리핑을 잡아서 해줍니다.
...
**직접 눌러야 하는 곳은 여기 하나뿐입니다.** 루틴 자체는 Claude가 만들어 주지만 폴더와 권한
모드는 대신 못 바꿉니다. 사이드바 **루틴** → 브리핑 → 상세 페이지를 열고 아래 세 가지를
확인하세요. 처음 한 번이면 끝나고, 어디를 눌러야 하는지는 세팅하면서 Claude가 짚어 줍니다.
