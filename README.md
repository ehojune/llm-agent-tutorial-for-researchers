...
2. **데일리 논문 브리핑** — 관심사를 바탕으로 매일 정해진 시간에 PubMed 최신 논문을 훑어
   브리핑합니다. **Claude의 예약 작업 기능**(사이드바 *Scheduled*)으로 돌아갑니다 — 세팅할
   때 Claude가 작업을 대신 등록해 주고, 그 뒤로는 사이드바에서 시간을 바꾸거나 잠시 꺼둘 수
   있습니다. 수신 방법은 데스크탑 알림(기본, 설정 불필요), 이메일, Slack, Notion 중 선택.
   예약 작업은 **앱이 켜져 있을 때** 돌기 때문에, 브리핑 시간에 컴퓨터가 꺼져 있었다면 다음에
   위키 폴더에서 Claude를 열었을 때 밀린 브리핑을 잡아서 해줍니다.
