...
| 시간 | 커밋 | 주요 변경사항 |
|---|---|---|
| PENDING | [`PENDING`](https://github.com/ehojune/llm-agent-tutorial-for-researchers/commit/PENDING) | **Windows에서 권한 목록에 `PowerShell`을 넣습니다.** `Bash`만 열어두면 PubMed 검색 루프가 매번 권한을 묻습니다 — Windows에서 그 루프는 PowerShell 도구로 돌기 때문입니다. 여러 줄 스크립트라 "항상 허용"도 안 뜨고 "한 번만 허용"만 나옵니다 |
