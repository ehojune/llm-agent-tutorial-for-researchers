...
| 시간 | 커밋 | 주요 변경사항 |
|---|---|---|
| PENDING | [`PENDING`](https://github.com/ehojune/llm-agent-tutorial-for-researchers/commit/PENDING) | 리뷰 지적 세 건 반영. Python은 3.6 이상만 받습니다 — 3.5도 "3으로 시작"이라 통과한 뒤 f-string에서 죽습니다. 계정이 **자동** 모드를 못 쓰면 "편집 자동 수락"으로 물러서지 않고, 무인 브리핑이 불가능하다고 알린 뒤 "브리핑 해줘"를 기본으로 안내합니다 — 물러서봐야 첫 셸 명령에서 멈추니까요. 그리고 명령 작성 규칙의 예시에서 Python 전용 표현을 뺐습니다 (원본판은 Python이 없을 수 있음) |
